# -*- coding: utf-8 -*-
"""
Created on Wed Jul 18 15:06:41 2018
...
files_training2 = np.array(sorted(files_training))
files_testing2 = np.array(sorted(files_testing))
...
data_training2 = np.empty(shape=[0,87,100]) 
label_training2 = np.empty(shape=[0,1])

for f2 in files_training2:
    img = cv2.imread(f2, cv2.IMREAD_GRAYSCALE)
    resized = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
    data_training2 = np.concatenate((data_training2, [resized]))
    label_training2 = np.append(label_training2,f2[-11:-8])


data_testing = np.empty(shape=[0,87,100]) 
label_testing = np.empty(shape=[0,1])

for f2 in files_testing2:
    img = cv2.imread(f2, cv2.IMREAD_GRAYSCALE)
    resized = cv2.resize(img, dim, interpolation = cv2.INTER_AREA)
    data_testing = np.concatenate((data_testing, [resized]))
    label_testing =  np.append(label_testing,f2[-11:-8])


del data_path, img, img_dir, f2, r, dim,i,testing_file_1,testing_file_2,resized
...
img_dir2 = "C:\\Users\\AH\\Desktop\\Iris\\Segmented_irises" # Enter Directory of all images 
data_path2 = os.path.join(img_dir2,'*mat')
files2 = glob.glob(data_path2)

#Load image
import scipy.io as sio

data_trainingS = np.empty(shape=[0,280,320]) 
label_trainingS = np.empty(shape=[0,1])
for f2 in files2:
    img = sio.loadmat(f2)
    data_trainingS = np.concatenate((data_trainingS, [img['outputimg']]))
    label_trainingS = np.append(label_trainingS,f2[-24:-17])


#%%
#Normalised images

img_dir3 = "C:\\Users\\AH\\Desktop\\Iris\\Normalised" # Enter Directory of all images 
data_path3 = os.path.join(img_dir3,'*mat')
files3 = glob.glob(data_path3)

#Load image
import scipy.io as sio

data_trainingN = np.empty(shape=[0,20,240]) 
label_trainingN = np.empty(shape=[0,1])
for f3 in files3:
    img = sio.loadmat(files3[0])
    data_trainingS = np.concatenate((data_trainingN, [img['polar_array']]))
    label_trainingS = np.append(label_trainingN,f3[-25:-18])


files_training3 = files3.copy()
files_testing3 =[]
for i in range(108):
    testing_file_3 = sorted(files3)[7*i][:-7]+'1_'+str(randint(1,3))+'.bmp'
    files_testing3.append(testing_file_3)
    files_training3.remove(testing_file_3)
    testing_file_3 = sorted(files3)[7*i][:-7]+'2_'+str(randint(1,4))+'.bmp'
    files_testing3.append(testing_file_3)
    files_training3.remove(testing_file_3)


files_training3 = np.array(sorted(files_training3))
files_testing3 = np.array(sorted(files_testing3))
...
def data_Shuffle(data, Label):
    s = np.arange(0, len(data), 1)
    np.random.shuffle(s)
    Label2 = Label[s]#[Label[index] for index in perm]
    data2 = data[s]#[data[index] for index in perm]
    del s
    return (data2/255,  Label2.astype('int32')-1)




trainx, trainy = data_Shuffle(data_training2, label_training2)
trainx2=trainx.reshape([trainx.shape[0], trainx.shape[1], trainx.shape[2], 1])
...
for e in range(epochs):
	...
	batches = 0
	per_batch = 10
	for x_batch, y_batch in datagen.flow(trainx2, trainy2, batch_size=per_batch):
...
for f1 in x_batches:
    ...
    cv2.imwrite("C:\\Users\\AH\\Desktop\\Iris\\test\\"+str(y_batches[z]+1)+"__"+str(z)+'.bmp',im)#f1[:,:,:])
    z=z+1
